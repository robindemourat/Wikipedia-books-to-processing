...
class Mot {
  ...
  color couleur = 0;
  int numReference = 0; //pour stocker les références des mots qui le nécessitent
  ...
  Mot(String t, float x, float y, PFont font, int size) {
    ...
    //tester si le mot contient un crochet - et donc une référence
    if(t.indexOf("[") >0){
      couleur = color(250, 0, 0);
      numReference = parseInt(t.substring(t.indexOf("[")+1, t.indexOf("]")));
    }
  ...
  void update() {
    ...
    //si le mot est survolé et cliqué, il devient actif
    if (highlight && !mousePressed) {
      motActif = t;
    }
  }
