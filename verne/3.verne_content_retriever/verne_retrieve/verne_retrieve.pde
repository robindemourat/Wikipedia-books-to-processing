...
void setup() {
  ...
  JSONObject meta = loadJSONObject("verne_refined_index.json").getJSONObject("meta");
  String url = meta.getString("url");
  output.setJSONObject("meta", meta);

  JSONArray contentGlobalOut = new JSONArray();

  JSONArray parts = loadJSONObject("verne_refined_index.json").getJSONArray("content");
  ...
  for (int i = 0; i < parts.size (); i++) {
    ...
    JSONArray contents = part.getJSONArray("content");
    ...
    part.setJSONArray("content", contentsOut);
    partsOut.append(part);
  }

  output.setJSONArray("content", partsOut);
  println("ok for saving");
  saveJSONObject(output, "output/verne_data.json");
  println("done saving");
