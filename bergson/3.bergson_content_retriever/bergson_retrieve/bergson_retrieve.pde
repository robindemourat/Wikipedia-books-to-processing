...
void setup() {
  ...
  JSONObject meta = loadJSONObject("bergson_refined_index.json").getJSONObject("meta");
  String url = meta.getString("url");
  output.setJSONObject("meta", meta);

  JSONArray contentGlobalOut = new JSONArray();

  JSONArray parts = loadJSONObject("bergson_refined_index.json").getJSONArray("content");
  JSONArray partsOut = new JSONArray();

  for (int i = 0; i < parts.size (); i++) {
    JSONObject part = parts.getJSONObject(i);
    part.setString("title", part.getString("title").replaceAll("\u2014", "-").replaceAll("\u2019", "'"));
  ...
  saveJSONObject(output, "output/bergson_data.json");
  println("done saving");
