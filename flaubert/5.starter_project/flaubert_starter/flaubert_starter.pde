...
void setup() {
  ...
  donnees = loadJSONObject("flaubert_data.json");
  ...
  //- si le paragraphe commence par un tiret considérer qu'il est spécial
  ...
  for (int j = 0; j < texte.size (); j++) {
    ...
    //tester si on est face à un dialogue
    Boolean paragraphIsSpecial = false;
    if(mots[0].charAt(0) == '-')
      paragraphIsSpecial = true;
    for (int i = 0; i < mots.length; i++) {

      //si la page n'est pas pleine, ajouter le mot
      if (!page.isFull) {
        page.ajouterMot(mots[i]);
        //si le paragraphe est spécial,alors le mot est spécial
        if(paragraphIsSpecial)
          page.mots.get(page.mots.size()-1).setSpecial(true);
